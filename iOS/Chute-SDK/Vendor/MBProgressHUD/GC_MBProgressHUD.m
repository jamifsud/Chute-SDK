...
@implementation GC_MBProgressHUD
...
+ (GC_MBProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	GC_MBProgressHUD *hud = [[GC_MBProgressHUD alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	...
	for (UIView *v in [view subviews]) {
		if ([v isKindOfClass:[GC_MBProgressHUD class]]) {
			viewToRemove = v;
		}
	}
	if (viewToRemove != nil) {
		GC_MBProgressHUD *HUD = (GC_MBProgressHUD *)viewToRemove;
